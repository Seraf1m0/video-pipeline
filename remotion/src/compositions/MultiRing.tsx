import React from "react";
import {
  AbsoluteFill, Audio, interpolate, spring,
  staticFile, useCurrentFrame, useVideoConfig,
  Easing, Sequence,
} from "remotion";
import { loadFont as loadSyne        } from "@remotion/google-fonts/Syne";
import { loadFont as loadMontserrat  } from "@remotion/google-fonts/Montserrat";
import { loadFont as loadBebasNeue   } from "@remotion/google-fonts/BebasNeue";
import { noise2D } from "@remotion/noise";
import { seededRand } from "../utils/seeded";

const { fontFamily: SYNE       } = loadSyne();
const { fontFamily: MONTSERRAT } = loadMontserrat();
const { fontFamily: BEBAS      } = loadBebasNeue();

export interface MultiRingProps {
  title?: string;
  rings: Array<{
    label: string;
    value: number;   // 0–100
    color?: string;
    suffix?: string;
  }>;
  accent_color?: string;
  duration_s?: number;
  bg_color?: string;
  seed?: number;
}

const EXIT_DUR = 44;

// Default per-ring color palette (outermost first)
const RING_COLORS = ["#A855F7", "#00C8FF", "#22D3EE", "#F97316", "#10B981"];

// Radii for up to 5 rings (outermost first)
const RING_RADII = [280, 220, 160, 100, 50];

const STROKE_W = 26;

// ─── BACKGROUND ──────────────────────────────────────────────────────────────
const Bg: React.FC<{ frame: number; color: string; total: number }> = ({ frame, color, total }) => {
  const inOp  = interpolate(frame, [0, 30], [0, 1], { extrapolateRight: "clamp" });
  const outOp = interpolate(frame, [total - EXIT_DUR, total - EXIT_DUR + 20], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  const op = inOp * outOp;
  const s1 = 1 + Math.sin(frame * 0.020) * 0.07;
  const nx  = noise2D("mrbx", frame * 0.0015, 0) * 5;
  const ny  = noise2D("mrby", 0, frame * 0.0015) * 4;

  return (
    <AbsoluteFill>
      <div style={{
        position: "absolute",
        left: `${50 + nx}%`, top: `${50 + ny}%`,
        transform: `translate(-50%,-50%) scale(${s1})`,
        width: 1000, height: 700, borderRadius: "50%",
        background: `radial-gradient(ellipse, ${color} 0%, transparent 65%)`,
        filter: "blur(130px)", opacity: op * 0.10,
      }} />
      <div style={{
        position: "absolute", inset: 0,
        background: "radial-gradient(ellipse 90% 85% at 50% 50%, transparent 20%, #000000DD 100%)",
      }} />
    </AbsoluteFill>
  );
};

// ─── GRID ────────────────────────────────────────────────────────────────────
const Grid: React.FC<{ frame: number; color: string; total: number }> = ({ frame, color, total }) => {
  const inOp  = interpolate(frame, [8, 28], [0, 1], { extrapolateRight: "clamp" });
  const outOp = interpolate(frame, [total - EXIT_DUR, total - EXIT_DUR + 16], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  const op = inOp * outOp * 0.06;
  return (
    <AbsoluteFill style={{ opacity: op }}>
      {Array.from({ length: 9 }, (_, i) => (
        <div key={`v${i}`} style={{
          position: "absolute", left: `${(i / 8) * 100}%`, top: 0, bottom: 0, width: 1,
          background: `linear-gradient(to bottom, transparent, ${color}44, transparent)`,
        }} />
      ))}
      {Array.from({ length: 6 }, (_, i) => (
        <div key={`h${i}`} style={{
          position: "absolute", top: `${(i / 5) * 100}%`, left: 0, right: 0, height: 1,
          background: "linear-gradient(to right, transparent, #FFFFFF10, transparent)",
        }} />
      ))}
    </AbsoluteFill>
  );
};

// ─── SCAN LINE ───────────────────────────────────────────────────────────────
const ScanLine: React.FC<{ frame: number; color: string }> = ({ frame, color }) => {
  const y  = interpolate(frame, [0, 20], [0, 110], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.out(Easing.cubic),
  });
  const op = interpolate(frame, [0, 4, 16, 24], [0, 1, 1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  return (
    <div style={{
      position: "absolute", top: `${y}%`, left: 0, right: 0, height: 2,
      background: `linear-gradient(to right, transparent, ${color}CC, transparent)`,
      boxShadow: `0 0 22px ${color}88`, opacity: op,
    }} />
  );
};

// ─── EXIT SCAN ───────────────────────────────────────────────────────────────
const ExitScan: React.FC<{ frame: number; total: number; color: string }> = ({ frame, total, color }) => {
  const start = total - EXIT_DUR + 10;
  const y  = interpolate(frame, [start, start + 18], [110, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.in(Easing.cubic),
  });
  const op = interpolate(frame, [start, start + 3, start + 14, start + 22], [0, 1, 1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  return (
    <div style={{
      position: "absolute", top: `${y}%`, left: 0, right: 0, height: 2,
      background: `linear-gradient(to right, transparent, ${color}88, transparent)`,
      boxShadow: `0 0 16px ${color}55`, opacity: op,
    }} />
  );
};

// ─── RING SVG — relative 620×620 box, cx=cy=310 ─────────────────────────────
const SVG_SIZE = 620;
const CX = SVG_SIZE / 2;
const CY = SVG_SIZE / 2;

const RingsSVG: React.FC<{
  rings: MultiRingProps["rings"];
  progresses: number[];
  exitProgress: number;
}> = ({ rings, progresses, exitProgress }) => {
  return (
    <svg width={SVG_SIZE} height={SVG_SIZE} viewBox={`0 0 ${SVG_SIZE} ${SVG_SIZE}`}>
      <defs>
        {rings.map((_, i) => (
          <filter key={i} id={`ring-glow-${i}`} x="-50%" y="-50%" width="200%" height="200%">
            <feGaussianBlur stdDeviation="6" result="blur" />
            <feMerge>
              <feMergeNode in="blur" />
              <feMergeNode in="SourceGraphic" />
            </feMerge>
          </filter>
        ))}
      </defs>

      {rings.map((ring, i) => {
        const r     = RING_RADII[i];
        const color = ring.color ?? RING_COLORS[i] ?? "#FFFFFF";
        const prog  = Math.min(progresses[i] ?? 0, 1);
        const circ  = 2 * Math.PI * r;

        const fillFraction = (ring.value / 100) * prog;
        const dashOffset   = circ * (1 - fillFraction);

        const exitScale = interpolate(exitProgress, [0, 1], [1, 0.7], { extrapolateRight: "clamp" });
        const exitOp    = interpolate(exitProgress, [0, 1], [1, 0],   { extrapolateRight: "clamp" });

        const tipAngle = -Math.PI / 2 + 2 * Math.PI * fillFraction;
        const tipX = CX + r * Math.cos(tipAngle);
        const tipY = CY + r * Math.sin(tipAngle);

        return (
          <g
            key={i}
            transform={`translate(${CX} ${CY}) scale(${exitScale}) translate(${-CX} ${-CY})`}
            opacity={exitOp}
          >
            <circle cx={CX} cy={CY} r={r} fill="none" stroke="#FFFFFF10" strokeWidth={STROKE_W} />
            <circle
              cx={CX} cy={CY} r={r}
              fill="none"
              stroke={color}
              strokeWidth={STROKE_W}
              strokeLinecap="round"
              strokeDasharray={circ}
              strokeDashoffset={dashOffset}
              transform={`rotate(-90 ${CX} ${CY})`}
              style={{ filter: `drop-shadow(0 0 10px ${color}99)` }}
            />
            {prog > 0.02 && (
              <circle
                cx={tipX} cy={tipY} r={8}
                fill={color}
                style={{ filter: `drop-shadow(0 0 12px ${color}CC)` }}
              />
            )}
          </g>
        );
      })}
    </svg>
  );
};

// ─── LABEL ROW ───────────────────────────────────────────────────────────────
const LabelRow: React.FC<{
  ring: MultiRingProps["rings"][number];
  progress: number;
  frame: number;
  enterFrame: number;
  exitStart: number;
  fps: number;
  index: number;
  totalRings: number;
}> = ({ ring, progress, frame, enterFrame, exitStart, fps, index, totalRings }) => {
  const color = ring.color ?? RING_COLORS[index] ?? "#FFFFFF";

  // Slide in from right
  const spr   = spring({ frame: frame - enterFrame, fps, config: { damping: 24, stiffness: 200 } });
  const xIn   = interpolate(spr, [0, 1], [140, 0]);
  const opIn  = interpolate(frame, [enterFrame, enterFrame + 16], [0, 1], { extrapolateRight: "clamp" });

  // Exit: slide right
  const exitDelay = index * 6;
  const opOut  = interpolate(frame, [exitStart + exitDelay, exitStart + exitDelay + 18], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  const xOut   = interpolate(frame, [exitStart + exitDelay, exitStart + exitDelay + 18], [0, 280], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp", easing: Easing.in(Easing.cubic),
  });

  const op = opIn * (frame >= exitStart ? opOut : 1);
  const x  = xIn + (frame >= exitStart ? xOut : 0);

  // Counter
  const displayValue = Math.round(interpolate(progress, [0, 1], [0, ring.value], {
    extrapolateRight: "clamp",
  }));

  return (
    <div style={{
      display: "flex",
      flexDirection: "column",
      gap: 4,
      opacity: op,
      transform: `translateX(${x}px)`,
      borderLeft: `3px solid ${color}66`,
      paddingLeft: 20,
    }}>
      {/* Label */}
      <div style={{
        fontFamily: MONTSERRAT,
        fontWeight: "700",
        fontSize: 14,
        letterSpacing: "0.12em",
        textTransform: "uppercase",
        color: `${color}CC`,
        lineHeight: 1,
      }}>
        {ring.label}
      </div>

      {/* Value + suffix */}
      <div style={{ display: "flex", alignItems: "baseline", gap: 8 }}>
        <span style={{
          fontFamily: BEBAS,
          fontSize: 80,
          lineHeight: 0.9,
          color: color,
          textShadow: `0 0 40px ${color}77`,
          letterSpacing: "0.02em",
        }}>
          {displayValue}
        </span>
        {ring.suffix && (
          <span style={{
            fontFamily: MONTSERRAT,
            fontWeight: "700",
            fontSize: 20,
            color: `${color}AA`,
            letterSpacing: "0.05em",
          }}>
            {ring.suffix}
          </span>
        )}
      </div>
    </div>
  );
};

// ─── MAIN ────────────────────────────────────────────────────────────────────
export const MultiRing: React.FC<MultiRingProps> = ({
  title,
  rings,
  accent_color = "#A855F7",
  duration_s   = 12,
  bg_color     = "#020218",
  seed         = 0,
}) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const totalFrames = Math.round(duration_s * fps);
  const exitStart   = totalFrames - EXIT_DUR;

  const rand = seededRand(seed);
  const _u = rand();

  const fadeIn    = interpolate(frame, [0, 10], [0, 1], { extrapolateRight: "clamp" });
  const finalFade = interpolate(frame, [totalFrames - 6, totalFrames], [1, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });

  // Clamp rings to max 5
  const activeRings = rings.slice(0, 5);

  // Per-ring spring progresses
  const RING_START  = 16;
  const RING_STEP   = 18;

  const progresses = activeRings.map((_, i) =>
    Math.min(
      spring({
        frame: frame - (RING_START + i * RING_STEP),
        fps,
        config: { damping: 30, stiffness: 65, mass: 1.2 },
      }),
      1
    )
  );

  // Exit fade for rings
  const exitProgress = interpolate(frame, [exitStart, exitStart + EXIT_DUR], [0, 1], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });

  // Title
  const TITLE_ENTER = 8;
  const titleOp = interpolate(frame, [TITLE_ENTER, TITLE_ENTER + 16], [0, 1], { extrapolateRight: "clamp" }) *
                  interpolate(frame, [exitStart + 4, exitStart + 20], [1, 0], {
                    extrapolateLeft: "clamp", extrapolateRight: "clamp",
                  });


  // SFX volumes per ring
  const RING_VOLS = [0.14, 0.11, 0.09, 0.07, 0.06];

  return (
    <AbsoluteFill style={{ background: bg_color, overflow: "hidden" }}>
      <div style={{ opacity: fadeIn * finalFade, width: "100%", height: "100%" }}>
        <Bg frame={frame} color={accent_color} total={totalFrames} />
        <Grid frame={frame} color={accent_color} total={totalFrames} />
        <ScanLine frame={frame} color={accent_color} />
        <ExitScan frame={frame} total={totalFrames} color={accent_color} />

        {/* SFX */}
        <Sequence from={0} durationInFrames={20}>
          <Audio src={staticFile("sfx/rise.wav")} volume={0.18} />
        </Sequence>
        {activeRings.map((_, i) => (
          <Sequence key={i} from={RING_START + i * RING_STEP + 4} durationInFrames={20}>
            <Audio src={staticFile("sfx/ping.wav")} volume={RING_VOLS[i] ?? 0.06} />
          </Sequence>
        ))}
        <Sequence from={exitStart + 6} durationInFrames={28}>
          <Audio src={staticFile("sfx/whoosh_out_3.wav")} volume={0.20} />
        </Sequence>

        {/* Центрированный flex-row: кольца + лейблы */}
        <AbsoluteFill style={{
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
        }}>
          <div style={{
            display: "flex",
            flexDirection: "row",
            alignItems: "center",
            gap: 80,
          }}>
            {/* Rings SVG — 620×620 relative box */}
            <div style={{ width: SVG_SIZE, height: SVG_SIZE, flexShrink: 0 }}>
              <RingsSVG
                rings={activeRings}
                progresses={progresses}
                exitProgress={exitProgress}
              />
            </div>

            {/* Labels panel */}
            <div style={{
              display: "flex",
              flexDirection: "column",
              justifyContent: "center",
              gap: activeRings.length <= 2 ? 48 : activeRings.length <= 3 ? 36 : activeRings.length <= 4 ? 24 : 16,
              minWidth: 320,
            }}>
              {/* Title */}
              {title && (
                <div style={{
                  opacity: titleOp,
                  fontFamily: SYNE,
                  fontSize: 12,
                  fontWeight: 800,
                  letterSpacing: "0.35em",
                  textTransform: "uppercase",
                  color: "#FFFFFF44",
                  marginBottom: 8,
                }}>
                  {title}
                </div>
              )}

              {activeRings.map((ring, i) => (
                <LabelRow
                  key={i}
                  ring={ring}
                  progress={progresses[i]}
                  frame={frame}
                  enterFrame={RING_START + i * RING_STEP}
                  exitStart={exitStart}
                  fps={fps}
                  index={i}
                  totalRings={activeRings.length}
                />
              ))}
            </div>
          </div>
        </AbsoluteFill>
      </div>
    </AbsoluteFill>
  );
};
